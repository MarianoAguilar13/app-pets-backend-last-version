...
//este codigo sirve para crear las tablas de la bd la primera vez
//luego desactivar para que no te sobreescriba todo

sequelize.sync({ alter: true }).then(() => {
  console.log("Se sincronizo la DB");
});*/

const port = process.env.PORT || 3000 || 5432;
const app = express();
...
app.post("/pets", authMiddleware, async (req, res) => {
  const id = req._user.id;
  const { name, type, description, localidad, provincia, lost } = req.body;
...
app.patch("/pets", authMiddleware, async (req, res) => {
  ...
  const { name, type, description, localidad, provincia, lost, petId } =
    req.body;
...
app.get("/pets/cercanas", async (req, res) => {
  ...
  if (localidad) {
    const petsResultado = await petsByLocalidad(localidad);

    //console.log(petsResultado);
    ...
    if (petsResultado[0]) {
      ...
      res.json({
        message:
          "No se han encontrado mascotas perdidas en la ubicación determinada",
      });
    }
  } else {
